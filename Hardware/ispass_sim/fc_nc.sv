      ...
      case (fsm_state)
        ...
        1: begin // accum add
            ...
            if(en_activ) begin
                ...
                membr_pot_nxt = membr_pot*LAP_TERM;
            end else begin
                intermed_pot = membr_pot + bram_rdat*w_sfactor; // scaling weight from int4 to fp32
                membr_pot_nxt = intermed_pot;
                bram_raddr = (neuron)*IN_CHANNELS*INPUT_FRAME_SIZE + spk_addr;
            end
        end
        2: begin
            membr_pot_nxt = membr_pot + bram_rdat*b_sfactor;
            fsm_state_nxt = 3;
        end
        3: begin
           if(membr_pot > signed_POSITIVE_THRESHOLD) begin 
                post_syn_spk_nxt = 1;
                membr_pot_nxt = 0;
                //membr_pot_nxt = (membr_pot - signed_POSITIVE_THRESHOLD)*NEURON_BETA;
            end else begin
                post_syn_spk_nxt = 0;
            end
