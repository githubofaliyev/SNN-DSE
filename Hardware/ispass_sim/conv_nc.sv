module conv_nc #(
    ...
    parameter BETA = 0.24,
    parameter signed_POSITIVE_THRESHOLD = 0.23,
  ...
  // LAP has this additional term called ``input decay" --> U[t+1] = β⋅I_in[t+1]+(1-β)⋅U[t]
  shortreal NEURON_BETA = (NEURON_TYPE == 1) ? (1-BETA): BETA;
  shortreal LAP_TERM = (NEURON_TYPE == 1) ? BETA : 1.0;
      ...
      case (fsm_state)
        ...
        1: begin 
           ...
           if(accum_membr_calc_stage) begin
                ...
                intermed_pot = bram_rdat + weight_data*w_sfactor;
//                intermed_pot = bram_rdat + (weight_n_bias[oc_phase * (IN_CHANNELS * KERNEL_SIZE * KERNEL_SIZE + 1) + ic * KERNEL_SIZE * KERNEL_SIZE + filter_phase] - w_zpt)*w_sfactor;
        ...
        3: begin // data read, now do the bias addition
            ...
            membr_pot_nxt = bias_data*b_sfactor  + bram_rdat*LAP_TERM;
            ...
            if(activ_membr_calc_stage) begin
                ...
                if(membr_pot > signed_POSITIVE_THRESHOLD) begin
                    ...
                    if(last_time_step) bram_wrdat = 0;
                        ...
                        bram_wrdat = (membr_pot - signed_POSITIVE_THRESHOLD)*NEURON_BETA;
                end else begin
                    post_syn_spk_nxt[intermed_addr] = 0;
                    if(last_time_step) bram_wrdat = 0;
                    else bram_wrdat = membr_pot*NEURON_BETA;
                end
            end
        end
        6: fsm_state_nxt = 0;
