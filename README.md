...
- `sparse_synth_int` Use for synhtesis to determine energy usage of rate coded models(INT4 Only). _There is no FP32 equivalent as no FP32 rate coded model was tested.
...
> [!IMPORTANT]
> `-d SIM` must be added to the `xsim.compile.xvlog.more_options*` field in the simulation project settings. This is to run the simulation path of the code. The simulation will fail otherwise.
